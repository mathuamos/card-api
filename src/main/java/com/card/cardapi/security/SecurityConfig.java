...
import com.card.cardapi.security.filter.TokenAuthenticationFilter;
import com.card.cardapi.services.TokenHelper;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public SecurityConfig(UserDetailsService userDetailsService, BCryptPasswordEncoder bCryptPasswordEncoder, AppFunctions appFunctions, TokenHelper tokenHelper, UserServiceImpl userServiceImpl) {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests().antMatchers("/api/v1/login/**","/api/token/refresh","/swagger-ui/*","/swagger-ui/**","/v3/api-docs/**").permitAll();
       ...
       // http.authorizeRequests().antMatchers(POST,"/api/user/save/**").hasAuthority("ROLE_USER");
        ...
        http.addFilterBefore(new TokenAuthenticationFilter(tokenHelper,userServiceImpl), UsernamePasswordAuthenticationFilter.class);
